...
| E7 | support bundle / doctor | `musializer_doctor.py` checks credential **membership** only (`"OPENROUTER_API_KEY" in environment`), never the value. The bundle collector applies the same name-marker strip before capturing any environment, and a canary test asserts it |
