# Feature-parity waves, 2026-08-01 to 2026-08-27 (archived)

**Archived 2026-08-29. This is history, not a queue.** It is the complete record
of the waves that took the Rust application from "the port compiles" to a tool
well past what the frozen C could do: the operator rounds (LX, EX, PX, CX, HX),
the parity tranches (P0–P4, A–G), the assist and lyric-timing work (AP, LT1) and
the three post-legacy scene waves (SX1 Phosphor Dream, SX2 Clawd, SX3 the depth
rework).

**`../FEATURE_PARITY_PLAN.md` is authoritative about what is open.** The
checkboxes below are frozen as they stood on 2026-08-29 and several are wrong in
the *stale-open* direction. Do not read a `- [ ]` here as a task; read the body,
and check the tree. Known traps, so nobody re-derives them:

- **UX0-C07 appears twice.** The ticked one at "UX0-C07 — Tune randomize/mutate"
  carries **UX0-C06's** body (recent projects, PX4); the unticked one carries a
  body beginning "*Done (PX6)*" with its gate captures listed. Both are closed.
- **The verify-gate totals disagree five ways** — 19, 20, 21, 22 and 8 passed —
  because each was true on the day it was written and the script has changed
  underneath all of them. `tools/verify.sh` is the only answer.
- **"ten scenes" appears about ten times.** `SCENE_COUNT` is 12;
  `ORACLE_SCENE_COUNT` is 10, and that second number is the one the harnesses use.
- **"Start here next session", the "Ordered completion map" and the
  "Feature-surface ledger" are the three most stale sections.** All three were
  last true on 2026-08-07, before four waves landed. Their dependency *arrows*
  are still sound; their status columns are not.
- **Sections disagree with each other across time.** EX3 says no gate photographs
  an export frame; SX3 added exactly that gate. PX3 says the clip window is "a
  field move"; CX-2 later ruled a field move alone would ship defects. The later
  section wins.

What this file is worth keeping for, and why it is not simply deleted: the
**evidence**. Each wave records the operator's own words that opened it, the
negative control that proved the check had teeth, the defect the capture found
that the tests did not, and the reasoning behind every divergence the
application still carries. That is the material a later session would otherwise
pay to rediscover. `AGENTS.md`'s divergence table is the index into it.

Two of its front-matter pointers have moved since: the UX perspective review is
now `UX_PERSPECTIVE_REVIEW_2026-08-03.md` beside this file, and `REWRITE_PLAN.md`
is still at the repository root.

---

## Target and meaning of parity (as written, 2026-08-01)
...
**Source and scope:** `docs/archive/UX_PERSPECTIVE_REVIEW_2026-08-03.md`, reviewed at `d22f9de` while
...
| `docs/archive/UX_PERSPECTIVE_REVIEW_2026-08-03.md` | Evidence companion, fully normalized into next-task umbrella UX0; it is not a parallel queue |
