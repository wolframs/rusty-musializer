...
rewrite also writes `time` | `bass` | `mids` | `treble` | `balance`
post-legacy, which the C cannot read),
...
| `MUSIALIZER_LIVE_CATALOG_TEST` | `tests/test_provider_discovery.py`, `live_catalog_skip_reason()` | Opt-in for the **only** test in `tests/` that opens a socket: the live OpenRouter catalog fetch. Gated by assist audit finding B3 — the suite runs from `tools/support_bundle_check.sh` inside `tools/verify.sh`, so ungated it made every gate run disclose this machine's IP to openrouter.ai, silently | the test skips, naming the variable in its reason | **exact string compare to `"1"`**; every other value, including `"0"`, `""` and `"yes"`, leaves the gate closed |
