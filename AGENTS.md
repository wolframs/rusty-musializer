...
| — | `clawd:` report line — face, `amp`/`bass`, peak petal and which, beat count, bounce, drawn cats, smoke level, terminal state, daylight, `warmth`/`mouth`/`semantic` | Invented, same argument as `phosphor dream:` — a dead expression machine is a permanent smile, petals uncoupled from their bands are a symmetric flower, and an empty floor that should hold three cats photographs as a perfectly plausible cute picture. The headless gate asserts face is named, petal peak coupled, three default cats drew, and the frame is not dark at default daylight |
