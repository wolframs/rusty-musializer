...
test('timed lyric alternatives share the audio clock and show the proposed gap', async ({ page }) => {
  await page.getByLabel('Session').selectOption('timed-lyrics-e2e')
  await expect(page.getByRole('heading', {name: 'Timed lyrics fixture', exact: true})).toBeVisible()
  await expect(page.getByText('Reading waveform…')).toBeHidden()
  const seek = page.getByLabel('Seek time in seconds or mm:ss.mmm')
  await expect(page.getByTestId('playhead')).toHaveText('00:01.000')
  const jump = async (time: string) => { await seek.fill(time); await seek.press('Enter'); await expect(page.getByTestId('playhead')).toHaveText(`00:0${Number(time).toFixed(3)}`) }
  await jump('1.2')
  await expect(page.getByTestId('caption-1')).toHaveText('first second')
  await expect(page.getByTestId('caption-2')).toHaveText('first')
  await jump('1.4')
  await expect(page.getByTestId('caption-1')).toHaveText('first second')
  await expect(page.getByTestId('caption-2')).toHaveText('No line displayed at this playhead')
  await expect(page.getByText('00:26.400–00:26.600', {exact: true})).toBeVisible()
  await page.getByRole('button', {name: 'Seek to second', exact: true}).click()
  await expect(page.getByTestId('playhead')).toHaveText('00:01.600')
  await expect(page.getByTestId('caption-2')).toHaveText('second')
  await jump('1.9')
  await expect(page.getByTestId('caption-1')).toHaveText('No line displayed at this playhead')
