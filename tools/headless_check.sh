...
# ---------------------------------------------------------------------------
# First-run and entry points: recent projects, typed file-drop dispatch, and the
# ASCII image import/clear controls (UX0-C06, D1, D2).
#
# Three things here cannot be checked any other way.
#
#  - **The welcome screen is the surface this repository has already shipped
#    unreviewed twice.** It is the only screen every other capture in this file
#    misses, because they all pass a fixture. Its new recent column has three
#    distinct empty-looking states — no history, unreadable history, and a
#    history whose files have moved — and a blank column is indistinguishable
#    from a broken one, so each gets a frame *and* a report assertion.
#  - **Xvfb has no drag-and-drop.** The whole of D1 is a three-arm branch on a
#    file extension, and until `--ui-probe drop=` existed no check in this
#    repository could enter it. That is precisely the shape of the export
#    panel's SIZE row (EX1): a dispatch that reads correctly in the source while
#    sending everything to one arm, with every gate green.
#  - **A control nothing presses is not verified, and hovering it is not the
#    same check.** Every new control here is clicked, not hovered — and the
#    first run of these clicks earned it: the recent row's open target was the
#    full row width and is drawn first, so it claimed every press aimed at the
#    forget cross and *opened the project* instead. Highlighting was correct,
#    the cross was drawn in the right place, and nothing but an injected press
#    could tell that it was dead. EX1, one namespace later.
#
# Two rules this section had to learn the hard way, both worth keeping:
#
#  - **Every store-mutating capture gets its own copy of the store.** A shared
#    one is stateful across captures: the first successful forget empties the
#    list, and every later capture then draws no rows and asserts nothing while
#    still passing. A whole diagnostic sweep was wasted on that.
#  - **No capture may reach a real file picker.** `kdialog` is installed on this
#    machine and Xvfb *is* a reachable display, so a click on Import would draw a
#    modal on :77 and block until somebody answered it — a hang, not a failure.
#    The Import click runs with a PATH that contains neither backend, which makes
#    the refusal deterministic and still proves the button took the press.
# ---------------------------------------------------------------------------

echo "=== first-run entry points: recent projects, typed drop, ASCII import ==="
ENTRY_FAILED=0
ENTRY_DIR="$OUT_DIR/entry-points"
rm -rf "$ENTRY_DIR"
mkdir -p "$ENTRY_DIR"
# A PATH with no `kdialog` and no `zenity`, so a picker is refused rather than
# raised. The application is invoked by explicit path and needs nothing from it.
ENTRY_NO_DIALOG_PATH="$ENTRY_DIR/no-dialog-backend"
mkdir -p "$ENTRY_NO_DIALOG_PATH"
# Synthetic audio and a first-party image. No user content, per the repository
# rule; the logo is generated by tools/make_rust_logo.py and lives in the tree.
cp "$FIXTURE" "$ENTRY_DIR/source.wav"
cp resources/logo/logo-256.png "$ENTRY_DIR/picture.png"
...
set -e
if [ ! -f "$ENTRY_PROJECT" ]; then
    echo "FAIL: could not build the entry-point fixture project" >&2
    ENTRY_FAILED=1
fi

# Seeds a recent store. Ages are computed from the wall clock rather than pinned,
# so the *words* ("1 minute ago", "3 days ago") are the same on every run while
# the timestamps stay honest.
entry_seed_store() {
    # entry_seed_store DESTINATION [missing|present]
    local destination="$1" second="${2:-missing}"
    python3 - "$destination" "$second" "$ENTRY_PROJECT" "$ENTRY_DIR/second.musi" <<'PY'
import json, os, sys, time
destination, second, project, second_path = sys.argv[1:5]
now = int(time.time())
entries = [{"path": os.path.abspath(project), "name": "Night Drive",
            "opened_unix": now - 90}]
if second == "missing":
    entries.append({"path": "/nonexistent/lost-tape.musi", "name": "Lost Tape",
                    "opened_unix": now - 3 * 86400})
else:
    entries.append({"path": os.path.abspath(second_path), "name": "Second Take",
                    "opened_unix": now - 2 * 3600})
json.dump({"schema": "musializer.recent-projects/v1", "projects": entries},
          open(destination, "w"))
PY
}

entry_store_count() {
    python3 -c "import json,sys;print(len(json.load(open(sys.argv[1]))['projects']))" "$1"
}

# entry_capture NAME STORE [args...]
#
# STORE is the recent-projects file this run may read and write. `-` means "a
# path that does not exist", which is how the argv-guard check below proves that
# a batch run creates nothing.
entry_capture() {
    local name="$1" store="$2"
    shift 2
    local out="$ENTRY_DIR/$name.png"
    ...
    local status=$?
    set -e
    printf '%-26s exit=%-3s %-34s %s\n' "$name" "$status" \
        "$(sed -n 's/^recent: *//p' "$log")" \
        "$(sed -n 's/^drop probe: *//p' "$log")"
    if [ "$status" -ne 0 ] || [ ! -f "$out" ]; then
        echo "FAIL: $name did not produce a frame (exit $status)" >&2
        ENTRY_FAILED=1
        return 1
    fi
    return 0
...
# Asserts one `key: value` report line, exactly.
entry_expect() {
    # entry_expect NAME KEY EXPECTED
    local name="$1" key="$2" expected="$3"
    local actual
    actual="$(sed -n "s/^$key: *//p" "$ENTRY_DIR/$name.txt")"
    if [ "$actual" != "$expected" ]; then
        echo "FAIL: $name: $key is '$actual', expected '$expected'" >&2
        ENTRY_FAILED=1
    fi
}

# Asserts a report line contains a substring, for the ones that carry a path.
entry_expect_contains() {
    local name="$1" key="$2" needle="$3"
    local actual
    actual="$(sed -n "s/^$key: *//p" "$ENTRY_DIR/$name.txt")"
    case "$actual" in
        *"$needle"*) ;;
        *)
            echo "FAIL: $name: $key is '$actual', expected it to contain '$needle'" >&2
            ENTRY_FAILED=1
            ;;
    esac
}

echo "--- UX0-C06: the recent-project list ---"

# 1. A new user. The column says "projects you save will appear here" rather than
#    drawing nothing, and the store is writable but absent.
entry_capture "recent-empty" "$ENTRY_DIR/store-empty.json" || true
entry_expect "recent-empty" "recent" "0 entries, 0 missing, store=writable"

# 2. A returning user, with one project present and one moved. This is the frame
#    that has to be looked at, and the one the ink assertion below guards.
entry_seed_store "$ENTRY_DIR/store-populated.json"
entry_capture "recent-populated" "$ENTRY_DIR/store-populated.json" || true
entry_expect "recent-populated" "recent" "2 entries, 1 missing, store=writable"

# The column is text on a light surface, so "something was drawn" is a *dark*
# extreme, not a bright one. YMIN over the two seeded rows: an empty column is
# the bare 246-ish chrome, and any drawn glyph pulls the minimum far below it.
# Negative control: with `recent-empty`'s store this crop reads 246.
RECENT_INK="$(ffprobe -v error -f lavfi \
    -i "movie=$ENTRY_DIR/recent-populated.png,crop=300:100:946:220,signalstats" \
    -show_entries frame_tags=lavfi.signalstats.YMIN -of csv=p=0 2>/dev/null | head -1)"
RECENT_INK_EMPTY="$(ffprobe -v error -f lavfi \
    -i "movie=$ENTRY_DIR/recent-empty.png,crop=300:100:946:220,signalstats" \
    -show_entries frame_tags=lavfi.signalstats.YMIN -of csv=p=0 2>/dev/null | head -1)"
echo "recent column ink: populated YMIN=${RECENT_INK:-?} empty YMIN=${RECENT_INK_EMPTY:-?}"
if [ "${RECENT_INK%%.*}" -ge 120 ] 2>/dev/null; then
    echo "FAIL: the recent column drew no rows where two were stored" >&2
    ENTRY_FAILED=1
fi
if [ "${RECENT_INK_EMPTY%%.*}" -le "${RECENT_INK%%.*}" ] 2>/dev/null; then
    echo "FAIL: an empty recent column is as dark as a populated one, so the crop proves nothing" >&2
    ENTRY_FAILED=1
fi
...
printf '{ broken' >"$ENTRY_DIR/store-corrupt.json"
cp "$ENTRY_DIR/store-corrupt.json" "$ENTRY_DIR/store-corrupt.expected"
entry_capture "recent-corrupt" "$ENTRY_DIR/store-corrupt.json" || true
entry_expect "recent-corrupt" "recent" "0 entries, 0 missing, store=unavailable"
if ! cmp -s "$ENTRY_DIR/store-corrupt.json" "$ENTRY_DIR/store-corrupt.expected"; then
    echo "FAIL: a corrupt recent store was overwritten instead of preserved" >&2
    ENTRY_FAILED=1
fi

# 4. Clicking a row reopens its project. Row 0's seat is (1087, 242) at 1280x720;
#    `shell_layout`'s `the_recent_column_seats_whole_rows_inside_the_spare_region`
#    pins the arithmetic those coordinates come from.
entry_seed_store "$ENTRY_DIR/store-open.json"
entry_capture "recent-click-open" "$ENTRY_DIR/store-open.json" \
    --ui-probe "click=1087x242" || true
entry_expect_contains "recent-click-open" "project" "night-drive.musi"
entry_expect_contains "recent-click-open" "click probe" "claimed=0xc00000001"
if [ "$(entry_store_count "$ENTRY_DIR/store-open.json")" != "2" ]; then
    echo "FAIL: reopening a project changed the length of the recent list" >&2
    ENTRY_FAILED=1
fi

# 5. Clicking the forget cross removes the entry and shortens the stored list.
#    Its own copy of the store, because this one mutates it.
entry_seed_store "$ENTRY_DIR/store-forget.json"
entry_capture "recent-click-forget" "$ENTRY_DIR/store-forget.json" \
    --ui-probe "click=1237x242" || true
entry_expect_contains "recent-click-forget" "click probe" "claimed=0xc00000002"
entry_expect "recent-click-forget" "tracks" "0 open"
if [ "$(entry_store_count "$ENTRY_DIR/store-forget.json")" != "1" ]; then
    echo "FAIL: the forget cross did not shorten the persisted recent list" >&2
    ENTRY_FAILED=1
fi

# 6. The negative control. A press into the gutter left of the column claims
#    nothing — without it, a probe that pressed nothing would satisfy every
#    assertion above, since a no-op leaves the defaults.
entry_seed_store "$ENTRY_DIR/store-gap.json"
entry_capture "recent-click-gap" "$ENTRY_DIR/store-gap.json" \
    --ui-probe "click=935x242" || true
entry_expect "recent-click-gap" "click probe" "at=935x242 claimed=nothing"
entry_expect "recent-click-gap" "tracks" "0 open"
if [ "$(entry_store_count "$ENTRY_DIR/store-gap.json")" != "2" ]; then
    echo "FAIL: a press into the gutter changed the recent list" >&2
    ENTRY_FAILED=1
fi

# 7. A row whose file has moved *takes* the press and deliberately does nothing:
#    the requirement is that it offers removal rather than erroring, and raising
#    a notice the user can do nothing about, on every click, forever, is the
#    error. What it says is on the row itself, permanently.
entry_seed_store "$ENTRY_DIR/store-missing.json"
entry_capture "recent-click-missing" "$ENTRY_DIR/store-missing.json" \
    --ui-probe "click=1087x292" || true
entry_expect_contains "recent-click-missing" "click probe" "claimed=0xc00000003"
entry_expect "recent-click-missing" "tracks" "0 open"

# 8. A batch run must not write the operator's configuration. `--probe-frames`
#    means "render N frames and exit", which is not a session, and without this
#    guard every `--project` launch in this very file would append a scratch
#    fixture to the real ~/.config/musializer/recent.json.
entry_capture "recent-argv-guard" "$ENTRY_DIR/store-never-written.json" \
    --project "$ENTRY_PROJECT" || true
entry_expect "recent-argv-guard" "recent" "0 entries, 0 missing, store=writable"
if [ -e "$ENTRY_DIR/store-never-written.json" ]; then
    echo "FAIL: a --probe-frames run wrote the per-user recent-projects store" >&2
    ENTRY_FAILED=1
fi

echo "--- D1: typed file-drop dispatch ---"

# Each arm is asserted twice: which branch the dispatch chose, and what that
# branch then did. The first alone would pass on a classifier wired to nothing.

# A dropped project opens as a project, and lands in the recent list — which is
# the interactive half of check 8 above, and must still happen in a probe run.
entry_capture "drop-project" "$ENTRY_DIR/store-drop.json" \
    --ui-probe "drop=$PWD/$ENTRY_PROJECT" || true
entry_expect_contains "drop-project" "drop probe" "night-drive.musi as project"
entry_expect_contains "drop-project" "project" "night-drive.musi"
entry_expect "drop-project" "recent" "1 entries, 0 missing, store=writable"

# A dropped image with no track is *staged* for the next one (`plug.c:825-839`),
# which is a state that used to have no report line at all — "no track" cannot be
# told from the drop having been discarded.
entry_capture "drop-image-staged" "$ENTRY_DIR/store-ignored.json" \
    --ui-probe "drop=$PWD/$ENTRY_DIR/picture.png" || true
entry_expect_contains "drop-image-staged" "drop probe" "picture.png as image"
entry_expect_contains "drop-image-staged" "ascii" "staged 54x54 glyphs"
entry_expect_contains "drop-image-staged" "ascii" "(no track yet)"

# With a track it becomes that track's grid and selects ASCII Field — the
# oracle's `&&` (`plug.c:7552`), so the scene assertion is not decoration.
entry_capture "drop-image-track" "$ENTRY_DIR/store-ignored.json" \
    "$ENTRY_DIR/source.wav" --ui-probe "drop=$PWD/$ENTRY_DIR/picture.png" || true
entry_expect_contains "drop-image-track" "drop probe" "picture.png as image"
entry_expect_contains "drop-image-track" "ascii" "54x54 glyphs from"
entry_expect "drop-image-track" "scene" "ascii (ASCII Field)"

# Supported audio still loads, which is the arm that already worked and the one a
# rewrite of this dispatch is most likely to break.
entry_capture "drop-audio" "$ENTRY_DIR/store-ignored.json" \
    --ui-probe "drop=$PWD/$ENTRY_DIR/source.wav" || true
entry_expect_contains "drop-audio" "drop probe" "source.wav as audio file"
entry_expect_contains "drop-audio" "tracks" "1 open"

# An unsupported file is *attempted* as audio and refused, and the report names
# audio — "reported by its attempted type", which is the whole requirement. The
# oracle's else branch is audio too (`plug.c:7559`); a fourth "unsupported" arm
# would mean this application, not raylib, deciding what raylib can open.
entry_capture "drop-unsupported" "$ENTRY_DIR/store-ignored.json" \
    --ui-probe "drop=$PWD/$ENTRY_DIR/notes.txt" || true
entry_expect_contains "drop-unsupported" "drop probe" "notes.txt as audio file"
entry_expect "drop-unsupported" "tracks" "0 open"

echo "--- D2: the ASCII image import and clear controls ---"

# Empty: Import is drawn, Clear is not. Proved by pressing Clear's seat and
...
# reached without a dialog.
entry_capture "ascii-populated" "$ENTRY_DIR/store-ignored.json" \
    "$ENTRY_DIR/source.wav" --ascii-image "$ENTRY_DIR/picture.png" || true
entry_expect_contains "ascii-populated" "ascii" "54x54 glyphs from"
...
# welcome screen is what is drawn, and the report that the grid was kept.
entry_capture "ascii-staged-no-track" "$ENTRY_DIR/store-ignored.json" \
    --ui-probe "drop=$PWD/$ENTRY_DIR/picture.png" || true
entry_expect_contains "ascii-staged-no-track" "ascii" "staged 54x54 glyphs"

if [ "$ENTRY_FAILED" -ne 0 ]; then
    echo "FAIL: the first-run entry-point checks did not pass" >&2
    SWEEP_FAILED=1
fi
